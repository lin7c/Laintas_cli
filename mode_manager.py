...
_BUILTINS = {
    ...
    "study": {
        ...
        "instructions": (
            ...
            "5. Cite your source. When a memory rests on something you READ - a "
            "file, a specific function - pass those paths as mem.save's "
            "`evidence` ('src/auth.py' or 'src/auth.py:120-180'). Their content "
            "is fingerprinted at save time, and the memory is flagged STALE "
            "automatically the moment any of them changes, so the claim gets "
            "re-checked instead of quietly rotting. A memory with no evidence "
            "can never be re-verified.\n"
            "6. Confirm briefly. Tell the user what you saved (name + one-line "
            "summary) so they can correct you. Do not dump the full body back.\n"
            "7. End the turn with task_complete summarising what you captured.\n"
            "\n"
            ...
            "\n"
            "STALE MEMORIES. An entry marked STALE is one whose cited source "
            "changed since it was written: UNVERIFIED, not wrong. Do not act on "
            "it and do not delete it. Re-read the source, then either re-save it "
            "with fresh `evidence` (which clears the flag) or save the corrected "
