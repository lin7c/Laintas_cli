...
if [ "$INSTALL_MODE" = "linux" ]; then
    ...
    # Download tarball
    echo "  Downloading…"
    curl -fsSL "$BASE_URL/releases/v1.1/laintas-cli_linux.tar.gz" -o "$TMP_DIR/laintas-cli_linux.tar.gz"
    ...
    curl -fsSL "$BASE_URL/releases/v1.1/laintas_cli.exe" -o "$TMP_DIR/laintas_cli.exe"
