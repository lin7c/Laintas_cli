...
export default function RegisterPage() {
  ...
  function validate() {
    ...
    else if (form.password.length < 8) errs.password = '密码至少8个字符';
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (result?.error) {
        setServerError(result.error.message || '注册失败，请重试');
        setSubmitting(false);
        return;
      }
      navigate(redirect || '/settings', { replace: true });
      ...
      >
        <div className="rounded-2xl p-8" style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', backdropFilter: 'blur(24px)' }}>
          <div className="text-center mb-8">
            ...
            <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>创建你的账户</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input ref={nameRef} type="text" value={form.name}
                ...
                placeholder="你的用户名" autoComplete="name"
                style={{ background: 'var(--input-bg)', border: errors.name ? '1px solid var(--danger)' : '1px solid var(--input-border)', color: 'var(--text-primary)' }}
            ...
            <div>
              ...
              <input type="password" value={form.confirmPassword}
                ...
                placeholder="••••••••" autoComplete="new-password"
                style={{ background: 'var(--input-bg)', border: errors.confirmPassword ? '1px solid var(--danger)' : '1px solid var(--input-border)', color: 'var(--text-primary)' }}
                className="w-full px-4 py-2.5 rounded-xl text-sm outline-none transition-all duration-200 placeholder:text-[var(--text-muted)] focus:border-[var(--accent-soft)]" />
              {errors.confirmPassword && <p className="mt-1 text-xs" style={{ color: 'var(--danger)' }}>{errors.confirmPassword}</p>}
            </div>
            ...
            <button type="submit" disabled={submitting}
              ...
              {submitting ? (
                <span className="inline-flex items-center gap-2"><span className="w-4 h-4 border-2 border-current/20 border-t-current rounded-full animate-spin" />注册中...</span>
              ) : '注册'}
            </button>
          </form>
          ...
          <div className="mt-6">
            <div className="relative mb-5">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="px-3 py-0.5 rounded-full font-mono text-[0.65rem] tracking-wider" style={{ background: 'var(--bg-surface)', color: 'var(--text-muted)' }}>或通过第三方注册</span>
              </div>
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={async () => {
                await auth.signIn.social({ provider: 'google', callbackURL: redirect ? `${window.location.origin}/register?redirect=${encodeURIComponent(redirect)}` : '/settings' });
              }}
              ...
              <button type="button" onClick={async () => {
                await auth.signIn.social({ provider: 'github', callbackURL: redirect ? `${window.location.origin}/register?redirect=${encodeURIComponent(redirect)}` : '/settings' });
              }}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all active:scale-[0.98] cursor-pointer"
              style={{ background: 'var(--bg-raised)', border: '1px solid var(--border-strong)', color: 'var(--text-primary)' }}>
                <GitHubIcon /> GitHub
          ...
          <p className="mt-6 text-center text-sm" style={{ color: 'var(--text-tertiary)' }}>
            已有账号？{' '}
            <Link to={`/login${redirect ? `?redirect=${encodeURIComponent(redirect)}` : ''}`} className="font-medium transition-colors" style={{ color: 'var(--accent-soft)' }}>登录</Link>
          </p>
        </div>
      </motion.div>
    </div>
