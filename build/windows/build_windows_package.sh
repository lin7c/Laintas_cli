...
OUT_DIR="${WINDOWS_OUT_DIR:-$PROJECT_DIR/build/windows/dist}"
...
for command in docker gzip zip; do
...
mkdir -p "$WORK_DIR/rootfs" "$WORK_DIR/package" "$OUT_DIR"
cp "$LINUX_BINARY" "$WORK_DIR/rootfs/laintas-cli"
...
cp "$PROJECT_DIR/build/windows/install.cmd" "$WORK_DIR/package/install.cmd"
...
cp "$PROJECT_DIR/build/windows/README.txt" "$WORK_DIR/package/README.txt"

PACKAGE="$OUT_DIR/laintas-cli_windows_${ARCH}.zip"
rm -f "$PACKAGE"
(cd "$WORK_DIR/package" && zip -q -9 "$PACKAGE" \
  laintas-cli.exe laintas-rootfs.tar.gz install.cmd install.ps1 uninstall.ps1 README.txt)
