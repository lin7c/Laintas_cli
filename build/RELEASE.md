...
(4 h). The file URLs used to be stable (`/releases/latest/laintas-cli_linux.tar.gz`), so
...
1. Build/refresh artifacts into `latest/` (steps below).
...
not part of the routine "rebuild all 3" flow.
...
Takes a few minutes; run it in the background.
...
`SHA256SUMS.txt` (same 4-entry set the repo commits: the 3 tarballs +
...
( cd "$PUB" && sha256sum laintas-cli_linux.tar.gz laintas-cli_macos.tar.gz \
    laintas-cli_source.zip laintas-cli_0.1.4_amd64.deb > SHA256SUMS.txt )
...
Releases are committed directly to `main` — that is the repo's convention.
Don't branch for a release.
...
```bash
ls -la laintas_cli_download/public/releases/latest/      # timestamps fresh
...
- **PyInstaller spec files have no `__file__`.** The Linux spec
  (`build/linux/laintas_cli.spec`) uses relative `../../` paths instead of
  `__file__` (that raised `NameError` and the binary never built).
