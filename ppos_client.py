...
_SCOPE_KEYS = ("publish", "comment", "community_review", "platform_review")
...
def parse_local_images(markdown: str, markdown_path: Path) -> list[LocalAsset]:
    """Resolve safe local Markdown image targets within the document directory."""
    ...
    for match in _IMAGE_RE.finditer(markdown):
        ...
        if split.scheme or split.netloc or raw.startswith(("#", "/", "\\")):
            if split.scheme in ("http", "https", "data") or raw.startswith("#"):
                continue
            raise PPOSClientError(f"unsafe image reference: {raw}")
        ...
        try:
            ...
            raise PPOSClientError(f"local image cannot be resolved safely: {raw}") from exc
        try:
            candidate.relative_to(base)
        except ValueError as exc:
            raise PPOSClientError(f"image escapes Markdown directory: {raw}") from exc
        if not candidate.is_file():
            raise PPOSClientError(f"image is not a regular file: {raw}")
...
def read_markdown(path: str | Path) -> tuple[Path, str, list[LocalAsset]]:
    ...
    return source, markdown, parse_local_images(markdown, source)
...
class PPOSClient:
    ...
    def _request(self, method: str, endpoint: str, *, params: Optional[dict] = None,
        ...
        if response.status_code >= 300:
            ...
            raise PPOSClientError(f"PPOS request failed (HTTP {response.status_code}): {detail or 'unknown error'}")
    ...
    def read(self, kind: str, *, page: int = 1, page_size: int = 20,
        ...
        if kind not in ("account", "storage", "communities", "works", "status",
                        "community_review_queue", "platform_review_queue"):
            raise PPOSClientError(f"unknown PPOS read: {kind}")
        params = dict(filters or {})
        if kind in ("communities", "works", "community_review_queue", "platform_review_queue"):
            params.update(page=max(1, int(page)), page_size=max(1, min(MAX_PAGE_SIZE, int(page_size))))
        ...
        scope = {
            ...
            "communities": "communities:read", "works": "works:read",
        ...
        if "page_size" in params:
            params["limit"] = params.pop("page_size")
        result = self._request("GET", endpoint, params=params, scopes=(scope,))
        with _cache_lock:
    ...
    def publish_markdown(self, path: str | Path, *, community: str, self_score: float,
        ...
        preflight = self._write("publish-preflight", ENDPOINTS["publish_preflight"],
                                preflight_content, scopes=("works:write",))
        ...
        for asset in assets:
            upload = self._write("storage-presign", ENDPOINTS["presign"], {
                "content_type": asset.media_type, "is_video": False, "size": asset.size,
            ...
            response = self.requests.put(url, data=data, headers=upload_headers, timeout=30)
            if response.status_code >= 300:
                raise PPOSClientError(
                    f"asset upload failed for {asset.reference} (HTTP {response.status_code})")
            rewritten = rewritten.replace(asset.reference, public_url)
    ...
    def comment(self, work_id: str, comment: str, *, rating: Optional[int] = None,
        ...
        if rating is not None:
            body["rating"] = rating
        result = self._write("comment", ENDPOINTS["comments"].format(
            work_id=quote(work_id, safe="")), body, scopes=("comments:write",))
        if autonomous:
